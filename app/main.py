...
logger = logging.getLogger("Q.U.A.N.T")
...
def print_title():
    """Print the Q.U.A.N.T ASCII art title."""
    title = """
   ╔══════════════════════════════════════════════════════════╗
   ║                                                          ║
   ║         ██╗ █████╗ ██████╗ ██╗   ██╗██╗███████╗          ║
...
async def lifespan(app: FastAPI):
    ...
    logger.info("[CONFIG] Tavily API key: %s", "configured" if TAVILY_API_KEY else "Not configured")
...
async def chat_jarvis_stream(request: ChatRequest):
    ...
    logger.info(
        "[API /chat/jarvis/stream] Incoming | session_id=%s | message_len=%d | message=%.100s",
        request.session_id or "new", len(request.message), request.message
    )
...
async def get_chat_history(session_id: str):
    ...
    if not chat_service:
        raise HTTPException(status_code=503, detail="Chat service not intialized")
