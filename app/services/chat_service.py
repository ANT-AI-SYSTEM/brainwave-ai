...
class ChatService:
    ...
    def _safe_append(self, session_id: str, chunk):
        if isinstance(chunk, str):
            self.sessions[session_id][-1].content += chunk
        elif isinstance(chunk, (int, float)):
            self.sessions[session_id][-1].content += str(chunk)
    ...
    # ensure it's serializable
                    try:
                        ...
                        yield {"activity": {"event": "warning", "message": "Invalid chunk skipped"}}
                    continue
                ...
                self._safe_append(session_id, chunk)
                chunk_count += 1
    ...
    def process_jarvis_message_stream(
        ...
        def _run_brain():
            if self.brain_service and brain_idx is not None:
                qt, r, ms = self.brain_service.classify(
                    user_message,
                    chat_history,
                    key_index=brain_idx
                )
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            try:
                query_type, reasoning, brain_elapsed_ms = future_brain.result(
                ...
                query_type, reasoning, brain_elapsed_ms = (
                "realtime",
        ...
        yield {"activity": {"event": "routing", "route": query_type}}

        if query_type == "realtime" and search_payload:
            yield {"search_results": search_payload}

        yield {"activity": {"event": "streaming_started", "route": query_type}}  
        chunk_count = 0
        t0 = time.perf_counter()
