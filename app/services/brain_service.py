...
class BrainService:
    def __init__(self):
        ...
        if GROQ_API_KEYS:
            try:
                ...
                logger.info(
                    "[BRAIN] Groq brain initialized (model: %s) with %d key(s)",
                    GROQ_BRAIN_MODEL,
                    len(self._llms),
                )
    ...
    def classify(
        ...
        user_content = f"""Conversation so far:\n{context_block}\n\nUser message:\n{msg_preview} Classify the current message . Output ONLY: general or realtime"""
        ...
        try:
            ...
            logger.warning(
                "[BRAIN] Groq error after %d ms: %s. Defaulting to realtime.",
                elapsed_ms, e
            )
        ...
        if re.search(r"\brealtime\b", text):
            logger.info(
                "[BRAIN] Groq (key #%d) returned realtime in %d ms",
                key_index + 1, elapsed_ms
            )
            return ("realtime", REASONING_REALTIME, elapsed_ms)

        if re.search(r"\bgeneral\b", text):
            logger.info(
                "[BRAIN] Groq (key #%d) returned general in %d ms",
                key_index + 1, elapsed_ms
            )
            return ("general", REASONING_GENERAL, elapsed_ms)

        logger.warning(
            "[BRAIN] Unexpected output: %r in %d ms. Defaulting to realtime.",
            text[:100], elapsed_ms
        )
