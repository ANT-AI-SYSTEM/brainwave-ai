...
class RealtimeGroqService(GroqService):
    ...
    def _extract_search_query(
        ...
        if len(q) <= 60 and not any(
            p in q.lower() for p in (" it ", " that ", " him ", " her ", " them ")
        ):
            return q
        ...
        try:
            ...
            logger.warning("[REALTIME] Query extraction failed: %s", e)
    ...
    def stream_response_with_prefetched(
        ...
        try:
            ...
            logger.info(
                "[REALTIME] Stream completed (pre-fetched results)"
            )
