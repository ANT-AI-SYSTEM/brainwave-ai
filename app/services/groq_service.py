...
class GroqService:
    ...
    def __init__(self, vector_store_service: VectorStoreService):
        ...
        logging.info(f"Initialized GroqService with {len(GROQ_API_KEYS)} API Key(s)")
    ...
    def _invoke_llm(
        ...
        masked_all = ", ".join([_mask_api_key(GROQ_API_KEYS[i]) for i in keys_tried])
        ...
        try:
            ...
            try:
               ...
               return self._invoke_llm(prompt, messages, question)
                ...
                logging.warning("vector store retrieval failed, using empty context: %s", retrieval_err)
    ...
    def stream_response(self, question: str, chat_history: Optional[List[tuple]] = None) -> Iterator[str]:
        try:
            if "remember" in question.lower():
               self.memory_service.save_memory(question)
